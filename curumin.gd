...
# --- REFERÊNCIAS DE SOM (NOVO) ---
@onready var sfx_pulo: AudioStreamPlayer2D = get_node_or_null("SfxPulo")
@onready var sfx_dano: AudioStreamPlayer2D = get_node_or_null("SfxDano")
...
func _physics_process(delta: float):
	...
	if Input.is_action_just_pressed("pular"):
		if is_on_floor():
			...
			# Toca som normal
			if sfx_pulo: 
				sfx_pulo.pitch_scale = 1.0 # Tom normal
				sfx_pulo.play()
			...
			# Toca som um pouco mais agudo para diferenciar
			if sfx_pulo:
				sfx_pulo.pitch_scale = 1.2
				sfx_pulo.play()
...
func ativar_escudo():
	if is_instance_valid(escudo_ativo): return 
